...
router.post(
  "/",
  isAuthenticated,
  async (req: Request<CreateCartBody>, res: Response) => {
...
router.put(
  "/",
  isAuthenticated,
  async (req: Request<UpdateProductQuantity>, res) => {
    ...
    try {
      ...
      res.status(200).json({
        message: `Se agrego/removio ${quantity} en el producto ${product_id}.`,
      });
    } catch (error) {
      res.status(500).json({ error: `Error en el servidor: ${error}` });
    }
...
router.delete("/", isAuthenticated, async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ message: "Se eliminaron todos los productos del carrito." });
  } catch (error) {
    res.status(500).json({ error: `Error en el servidor: ${error}` });
  }
});
